    ...
    static void dumpOpenCLInformation() {
        ...
        try {
                ...
                for (size_t i = 0; i < platforms.size(); i++) {
                    ...
                    for (int j = 0; j < platform->deviceNumber(); j++) {
                        ...
                        const char *deviceTypeStr = current_device.type() == Device::TYPE_CPU
                                                    ? ("CPU") : (current_device.type() ==
                                                                 Device::TYPE_GPU
                                                                 ? current_device.hostUnifiedMemory()
                                                                   ? "iGPU" : "dGPU" : "unknown");
                        DUMP_MESSAGE_STDOUT(
                                "        " << deviceTypeStr << ": " << current_device.name().c_str()
                        ...
                        DUMP_CONFIG_PROPERTY(
                                ...
                                cv::format("(Platform=%s)(Type=%s)(Name=%s)(Version=%s)",
                                           platform->name().c_str(), deviceTypeStr,
            ...
            const char *deviceTypeStr = device.type() == Device::TYPE_CPU
                                        ? ("CPU") : (device.type() == Device::TYPE_GPU
                                                     ? device.hostUnifiedMemory() ? "iGPU" : "dGPU"
                                                     : "unknown");
